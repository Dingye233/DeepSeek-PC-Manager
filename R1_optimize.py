...
def collect_user_information(message:str):
    ...
    with open("user_information.txt", "r", encoding="utf-8") as file:
        content = file.read()
    messages = [{"role":"system","content":"请提取用户文本里面的关键信息，然后跟前面收集的信息做对比，查漏补缺,提取不到关键信息则输出一个空字符串"},{"role":"system","content":"这是前面已经收集的信息: "+content},{"role": "user", "content":"这是用户跟其他模型交互的文本，提取一些有用的信息以构建用户的个性化需求:" +message}]
